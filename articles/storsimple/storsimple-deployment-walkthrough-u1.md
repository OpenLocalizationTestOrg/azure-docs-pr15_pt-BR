...
A seguinte lista de verificação de configuração de implantação descreve as informações que você precisa reunir antes e como você configurar o software em seu dispositivo de StorSimple. Preparar algumas dessas informações antecedência ajudarão a simplificar o processo de implantação de dispositivo StorSimple no seu ambiente. Use esta lista de verificação para anote os detalhes de configuração também como implantar o seu dispositivo.

| Estágio                                  | Parâmetro                                         | Detalhes                                                                                                                                                                | Valores |
|----------------------------------------|---------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------|
| **Cabo seu dispositivo**                      | Acesso serial                                     | Configuração de dispositivo inicial                                                                  | Sim/não |
|   |   |  |  |
| **Configurar e registrar o dispositivo**          | Configurações de rede de dados 0                           | Endereço IP de dados 0:</br>Máscara de sub-rede:</br>Gateway:</br>Servidor DNS primário:</br>Servidor NTP primário:</br>Servidor de proxy Web IP/FQDN (opcional):</br>Porta de proxy da Web:|        |
|                                        | Senha de administrador do dispositivo                     | Senha deve ter entre 8 e 15 caracteres que contém caracteres em minúsculas, maiusculas, numérico e especial. |        |
|                                        | Gerenciador de instantâneo StorSimple senha              | Senha deve ser 14 ou 15 caracteres que contém caracteres em minúsculas, maiusculas, numérico e especial.|        |
|                                        | Chave do registro de serviço                          | Esta chave é gerada a partir do portal clássico Azure.    |        |
|                                        | Chave de criptografia de dados de serviço                       | Esta chave é criada quando o dispositivo é registrado com o serviço de gerenciamento através do Windows PowerShell para StorSimple. Copie esta tecla e salvá-lo em um local seguro.|  |
|   |   |  |  |
| **Configuração de dispositivo mínimos concluída**          | Nome amigável para o seu dispositivo                     | Este é um nome descritivo para o dispositivo. |        |
|                                        | Fuso horário                                          | Seu dispositivo usará este fuso horário para todas as operações agendadas.  |        |
|                                        | Servidor DNS secundário                              | Esta é uma configuração necessária.                                  |        |
|                                        | Interface de rede: controlador de dados 0 fixo IPs                                     | Estas IP deve ser roteáveis na Internet.</br>Controlador de 0 fixo IP endereço:</br>Controlador de 1 fixo IP endereço:|
|   |   |  |  |
| **Configurações de interface de rede adicionais**  | Interface de rede: dados 1</br>Se iSCSI habilitada, não configure o Gateway.      | Objetivo: Nuvem/iSCSI/não usado</br>Endereço IP:</br>Máscara de sub-rede:</br>Gateway:|
|                                        | Interface de rede: dados 2</br>Se iSCSI habilitada, não configure o Gateway.      | Objetivo: Nuvem/iSCSI/não usado</br>Endereço IP:</br>Máscara de sub-rede:</br>Gateway:|
|                                        | Interface de rede: dados 3</br>Se iSCSI habilitada, não configure o Gateway.      | Objetivo: Nuvem/iSCSI/não usado</br>Endereço IP:</br>Máscara de sub-rede:</br>Gateway:|
|                                        | Interface de rede: dados 4</br>Se iSCSI habilitada, não configure o Gateway.      | Objetivo: Nuvem/iSCSI/não usado</br>Endereço IP:</br>Máscara de sub-rede:</br>Gateway:|
|                                        | Interface de rede: dados 5</br>Se iSCSI habilitada, não configure o Gateway.      | Objetivo: Nuvem/iSCSI/não usado</br>Endereço IP:</br>Máscara de sub-rede:</br>Gateway:|
|   |   |  |  |
| **Criar um contêiner de volume**                      | Nome do contêiner de volume:                            | Nome para o recipiente                                                                                                                                                 |        |
|                                        | Conta de armazenamento do Azure:                            | Chave de acesso e o nome de conta de armazenamento para associar a este contêiner de volume                                                                                              |        |
|                                        | Chave de criptografia de armazenamento de nuvem:                     | Chave de criptografia para armazenamento em cada contêiner                                                                                                                           |        |
|   |   |  |  |
