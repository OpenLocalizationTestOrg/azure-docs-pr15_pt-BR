<properties 
   pageTitle="Gerenciar NSGs utilizando a CLI Azure no Gerenciador de recursos | Microsoft Azure"
   description="Saiba como gerenciar NSGs existentes usando a CLI Azure no Gerenciador de recursos"
...
# <a name="manage-nsgs-using-the-azure-cli"></a>Gerenciar NSGs utilizando a CLI do Azure
...
Para exibir a lista de NSGs em um grupo de recursos específica, execute o `azure network nsg list` comando conforme mostrado abaixo. 
...
Para exibir as regras de um NSG chamado **NSG FrontEnd**, execute o `azure network nsg show` comando conforme mostrado abaixo. 
...
>[AZURE.NOTE] Você também pode usar `azure network nsg rule list --resource-group RG-NSG --nsg-name NSG-FrontEnd` para listar as regras de **FrontEnd NSG** NSG.

### <a name="view-nsg-associations"></a>Exibir associações de NSG

Para exibir quais recursos o NSG **NSG FrontEnd** é associar, execute o `azure network nsg show` comando conforme mostrado abaixo. Observe que a única diferença é o uso do parâmetro **– json** .
...
Para adicionar uma regra permitindo tráfego de **entrada** à porta **443** de qualquer máquina para **FrontEnd NSG** NSG, execute o `azure network nsg rule create` comando conforme mostrado abaixo.
...
Para alterar a regra criada acima para permitir o tráfego de entrada de apenas à **Internet** , execute o `azure network nsg rule set` comando conforme mostrado abaixo.
...
Para excluir a regra criada acima, execute o `azure network nsg rule delete` comando conforme mostrado abaixo.
...
>[AZURE.NOTE] O **– silencioso** parâmetro garante que você não precisa confirmar a exclusão.
...
Para associar o **FrontEnd NSG** NSG para **TestNICWeb1** NIC, execute o `azure network nic set` comando conforme mostrado abaixo.
...
Para dissociar **FrontEnd NSG** NSG de **TestNICWeb1** NIC, execute o `azure network nic set` comando conforme mostrado abaixo.

    azure network nic set --resource-group RG-NSG --name TestNICWeb1 --network-security-group-id ""

>[AZURE.NOTE] Observe o "" valor (vazia) para o parâmetro de **rede-segurança-grupo-id** . É como você pode remover uma associação para um NSG. Você não pode fazer o mesmo com o parâmetro de **rede segurança-nome do grupo** .
...
Para dissociar **FrontEnd NSG** NSG da sub-rede **FrontEnd** , execute o `azure network vnet subnet set` comando conforme mostrado abaixo.
...
Para associar o **FrontEnd NSG** NSG à sub-rede **FronEnd** novamente, execute o `azure network vnet subnet set` comando conforme mostrado abaixo.
...
>[AZURE.NOTE] O comando acima só funciona porque o NSG **NSG FrontEnd** está no mesmo grupo de recursos como a rede virtual **TestVNet**. Se o NSG estiver em um grupo de recursos diferentes, você precisa usar o **-rede-segurança-grupo-id** parâmetro em vez disso e fornecer a identificação completa para o NSG. Você pode recuperar a identificação executando **nsg de rede azure Mostrar - grupo de recursos RG-NSG - nome NSG FrontEnd – json** e procurando a propriedade **id** . 
...
4. Para excluir o NSG, execute o `azure network nsg delete` comando conforme mostrado abaixo.
