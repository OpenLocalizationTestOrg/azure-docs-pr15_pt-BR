<properties 
    pageTitle="Implementando failover streaming cenário | Microsoft Azure" 
    description="Este tópico mostra como implementar failover streaming cenário." 
...
#<a name="implementing-failover-streaming-scenario"></a>Implementando failover streaming cenário

Este passo a passo demonstra como copiar o conteúdo (blobs) de um ativo para outra para lidar com redundância para streaming sob demanda. Esse cenário é útil para clientes que deseja configurar sua CDN para failover entre dois centros de dados no caso de uma interrupção em um dos nossos data centers.
Este passo a passo usa o SDK de serviços de mídia do Microsoft Azure, API REST de serviços de mídia do Microsoft Azure e SDK de armazenamento do Azure para demonstrar as seguintes tarefas.

1. Configure uma conta de serviços de mídia do "Data Center A".
1. Carregar um arquivo de mezzanine em um ativo de origem.
1. Codifica ativo nos arquivos de vários bits taxa MP4. 
1. Crie um localizador SAS somente leitura para o ativo de origem ter acesso de leitura para o contêiner na conta de armazenamento que está associada com o ativo de origem.
1. Obtenha o nome do contêiner do ativo fonte do localizador SAS de somente leitura criado na etapa anterior. Precisamos dessas informações para copiar blobs entre contas de armazenamento (explicadas mais tarde no tópico.)
1. Crie um localizador de origem para o ativo que foi criado pela tarefa codificação. 

Em seguida, para lidar com o failover:

1. Configure uma conta de serviços de mídia do "B do Centro de dados".
1. Crie um ativo de destino vazio no destino conta de serviços de mídia.
1. Crie um localizador SAS de gravação do ativo vazia de destino ter acesso de gravação para o contêiner de destino conta de armazenamento que está associado com o ativo de destino.
1. Usar o SDK de armazenamento do Azure para copiar blobs (arquivos de ativos) entre a conta de armazenamento de origem no "Data Center A" e a conta de armazenamento de destino em "B do Centro de dados" (essas contas de armazenamento são associadas com os ativos de interesse.)
1. Associe bolhas (arquivos de ativo) que foram copiadas para o contêiner de blob de destino com o ativo de destino. 
1. Crie um localizador de origem para o ativo no "B do Centro de dados" e especifique o localizador Id que foi gerado para o ativo no "Data Center A". 
1. Isso lhe as URLs streaming os caminhos relativos das URLs anotados a mesma (somente a base URLs são diferentes). 
 
Em seguida, para lidar com quaisquer interrupções, você pode criar uma CDN sobre esses locators de origem. 

Aplica considerando o seguinte:

- A versão atual do Media Services SDK não suporta a criação de um localizador com uma identificação de localizador especificada. Para obter esta tarefa, usaremos API de REST de serviços de mídia.
- A versão atual do Media Services SDK não suporta programaticamente gerando informações de IAssetFile que pode associar um ativo com arquivos de ativos. Para obter esta tarefa, usaremos a API REST de serviços de mídia de CreateFileInfos. 
- Ativos de armazenamento criptografado (AssetCreationOptions.StorageEncrypted) não são suportados para replicação (desde que a chave de criptografia será diferente em ambas as contas de serviços de mídia). 
- Se quiser tirar proveito de embalagem dinâmico, você deve primeiro obter Streaming unidades reservadas pelo menos um sob demanda. Para obter mais informações, consulte [Dinamicamente ativos de embalagem](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Considere o uso de serviços de mídia [Replicator ferramenta](http://replicator.codeplex.com/) como uma alternativa para implementar um cenário de streaming de failover manualmente. Esta ferramenta permite replicar ativos entre duas contas de serviços de mídia.

##<a name="prerequisites"></a>Pré-requisitos
 
- Duas contas de serviços de mídia em uma assinatura do Azure nova ou existente. Veja [como criar uma conta de serviços de mídia](media-services-portal-create-account.md).
- Sistemas operacionais: Windows 7, Windows 2008 R2 ou Windows 8.
- .NET framework 4,5 ou .NET Framework 4.
- Visual Studio 2010 SP1 ou posterior (Professional, Premium, Ultimate ou Express).
 
##<a name="set-up-your-project"></a>Configurar o seu projeto

Nesta seção, você irá criar e configurar um projeto de aplicativo de Console c#.

1. Use o Visual Studio para criar uma nova solução que contém o projeto de aplicativo de Console c#. Digite HandleRedundancyForOnDemandStreaming para o nome e clique em Okey.
1. Crie a pasta SupportFiles no mesmo nível do arquivo de projeto HandleRedundancyForOnDemandStreaming.csproj. Sob a pasta de SupportFiles crie as pastas OutputFiles e MP4Files. Copie um arquivo. mp4 para a pasta MP4Files (neste exemplo, que o arquivo BigBuckBunny.mp4 é usado). 
...
1. Adicionar outras referências necessários para esse projeto: System. Configuration, System.Runtime.Serialization e System. Web.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Adicione código que manipula redundância para streaming sob demanda.
...
1. Substitua a definição do método padrão principal com o seguinte:
...
1. Definições de métodos que são chamadas de principal estão definidas abaixo.
...
Agora você pode usar um Gerenciador de tráfego para rotear solicitações entre os dois data centers e, portanto, failover no caso de qualquer interrupções.
