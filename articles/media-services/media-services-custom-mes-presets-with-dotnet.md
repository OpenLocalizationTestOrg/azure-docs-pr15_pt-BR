<properties 
    pageTitle="Advanced codificação com mídia codificador padrão | Microsoft Azure" 
...
Esta seção fala sobre modificando predefinições codificador para cortar ou aparar o vídeo de entrada onde a entrada é um arquivo de mezzanine chamados ou sob demanda. O codificador também pode ser usado para cortar ou aparar um ativo que capturado ou arquivado a partir de um fluxo ao vivo – os detalhes para isso estão disponíveis [neste](https://azure.microsoft.com/blog/sub-clipping-and-live-archive-extraction-with-media-encoder-standard/)blog.

Para aparar vídeos, você pode executar qualquer uma da MES predefinições documentadas [aqui](https://msdn.microsoft.com/library/mt269960.aspx) e modifique o elemento de **fontes** (conforme mostrado abaixo). O valor da hora de início precisa coincidir com os carimbos de hora absolutos do vídeo de entrada. Por exemplo, se o primeiro quadro do vídeo de entrada tem um carimbo de hora do 12:00:10.000, em seguida, hora de início deve ter pelo menos 12:00:10.000 e maior. No exemplo abaixo, vamos supor que o vídeo de entrada tem um carimbo de hora inicial igual a zero. Observe que as **fontes** deve ser colocado na parte superior do esquema. 
...
>
>O arquivo de vídeo de origem e o arquivo de sobreposição têm que estar no mesmo ativo.
...
Por padrão, se você enviar uma entrada com o codificador que contenha apenas vídeo e nenhum áudio, o ativo de saída conterá arquivos que contêm dados somente vídeo. Alguns players podem não ser capazes de lidar com tais fluxos de saída. Você pode usar esta configuração para forçar o codificador para adicionar uma faixa de áudio silenciosa na saída nesse cenário.
