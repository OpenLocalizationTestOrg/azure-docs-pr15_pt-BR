<properties
   pageTitle="Exemplo de configuração de extensões de máquina virtual do Windows | Microsoft Azure"
...
# <a name="azure-windows-vm-extension-configuration-samples"></a>Exemplos de configuração de extensão de máquina virtual do Windows Azure
...
Este artigo fornece um exemplo de configuração de configuração de Azure máquina virtual extensões para Windows máquinas virtuais.

Para saber mais sobre essas extensões, consulte [Visão geral de extensões de máquina virtual do Azure.](virtual-machines-windows-extensions-features.md)
...
Este artigo lista os valores de configuração esperadas algumas das extensões do Windows.

## <a name="sample-template-snippet-for-vm-extensions-with-iaas-vms"></a>Trecho de modelo de amostra para extensões de máquina virtual com VMs IaaS.
...
Restante do artigo fornece exemplos de configurações para extensões de máquina virtual do Windows.
...
- fileUris: lista de separada por vírgulas de urls dos arquivos que serão baixados na VM pela extensão. Não há arquivos são baixados se nada for especificado. Se os arquivos estiverem em armazenamento Azure, o fileURLs pode ser marcada como particular e a correspoding storageAccountName e storageAccountKey podem ser passados como parâmetros particulares para acessar esses arquivos.
- commandToExecute: [parâmetro obrigatório]: esse é o comando que será executado pela extensão.
- storageAccountName: [parâmetro opcional]: nome de conta de armazenamento para acessar o fileURLs, se eles estiverem marcados como particulares.
- storageAccountKey: [parâmetro opcional]: chave da conta de armazenamento para acessar o fileURLs, se eles estiverem marcados como particulares.

### <a name="customscript-extension-17"></a>Extensão de CustomScript 1.7.

Consulte CustomScript versão 1,4 para descrição do parâmetro. Versão 1.7 apresenta suporte para o envio de script parameters(commandToExecute) como protectedSettings, caso em que eles serão criptografados antes de enviar. parâmetro de 'commandToExecute' pode ser especificado nas configurações ou protectedSettings, mas não em ambos.
...
### <a name="dsc-extension"></a>Extensão de DSC.
      {
...
### <a name="symantec-endpoint-protection"></a>Proteção de ponto de extremidade do Symantec.
      {
...
### <a name="trend-micro-deep-security-agent"></a>Agente de segurança profunda Micro tendência.
      {
...
### <a name="vormertric-transparent-encryption-agent"></a>Agente de criptografia transparente Vormertric.
            {
              "publisher": "Vormetric",
...
### <a name="puppet-enterprise-agent"></a>Agente de Enterprise puppet.
            {
...
### <a name="microsoft-monitoring-agent-for-azure-operational-insights"></a>Microsoft monitoramento agente para obtenção de informações operacionais Azure
            {
...
### <a name="confer-advanced-threat-prevention-and-incident-response-for-azure"></a>Conferência prevenção de ameaças avançadas e resposta a incidente do Azure
          {
            "publisher": "Confer",
            "type": "ConferForAzure",
            "typeHandlerVersion": "1.0",
...
### <a name="alert-logic-log-manager"></a>Gerenciador de Log de lógica de alerta
          {
...
Para obter mais detalhes sobre como configurar diagnósticos, consulte [Extensão de diagnóstico do Azure](virtual-machines-windows-extensions-diagnostics-template.md)
...
Aqui está um exemplo de um modelo de máquina virtual completo com extensão de Script personalizado.
